...
try {
    ...
    // Verify listing type dropdown exists and select Rent
    WebUI.verifyElementPresent(findTestObject('Object Repository/PropertyService/SellPage/listingTypeSelect'), 10)
    WebUI.selectOptionByValue(findTestObject('Object Repository/PropertyService/SellPage/listingTypeSelect'), 'rent', false)
    WebUI.delay(1)
    
    WebUI.comment('✅ TC_PROP_021 PASSED: Rent listing type selected')
} catch (Exception e) {
    WebUI.comment('❌ TC_PROP_021 FAILED: ' + e.getMessage())
    throw e
} finally {
    WebUI.closeBrowser()
}
