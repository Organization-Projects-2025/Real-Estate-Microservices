...
/**
 * Test Case: TC_ADM_PROP_004 - Change Property Status
 * Module: Admin Service - Property Management
 * Priority: High
 * 
 * Description: Verify that admin can change a property's status (active/sold)
...
try {
    ...
    String currentStatus = WebUI.getAttribute(
        findTestObject('Object Repository/Admin/PropertiesPage/Modal/select_Status'), 'value'
    )
    String newStatus = currentStatus == 'active' ? 'sold' : 'active'
    WebUI.comment("Changing status from ${currentStatus} to ${newStatus}")
    
    // Change status
    WebUI.selectOptionByValue(
        findTestObject('Object Repository/Admin/PropertiesPage/Modal/select_Status'),
        newStatus, false
    )
    ...
    propertyHelper.assertSuccessToast("Status change failed: Expected success toast after changing status")
    
    // Assert: No error toast
    propertyHelper.assertNoErrorToast("Status change failed: Error toast appeared")
    
    WebUI.comment('✅ TC_ADM_PROP_004 PASSED')
    
} catch (Exception e) {
    WebUI.comment('❌ TC_ADM_PROP_004 FAILED: ' + e.getMessage())
    throw e
} finally {
    if (testPropertyId) {
