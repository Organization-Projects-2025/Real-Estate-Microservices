...
const Navbar = () => {
  ...
  const showAdminStrip = isAuthenticated && user?.role === 'admin' && !location.pathname.startsWith('/admin');
  ...
  return (
    <>
      ...
      <nav className={`bg-gradient-to-r from-[#141414] via-[#1a1a1a] to-[#141414] text-white py-4 px-4 sm:px-6 shadow-[0_4px_12px_rgba(112,59,247,0.2)] font-urbanist sticky z-[100] ${showAdminStrip ? 'top-8' : 'top-0'}`}>
        <div className="w-full mx-auto flex items-center justify-between relative">
          ...
          <ul className="hidden lg:flex gap-3 items-center text-base font-semibold pr-20">
            {navLinks.map((link) => (
              <li
                key={link.name}
          ...
          <ul className="hidden lg:flex gap-3 items-center text-base font-semibold pl-20">
            {authLinks.map((link) => (
              <li
                key={link.name}
                onClick={link.action || (() => navigate(link.path))}
                className={`relative px-3.5 py-2.5 transition-all duration-300 cursor-pointer group ${link.isButton
              ...
              >
                ...
                {link.name}
                <span
                  className={`absolute bottom-0 left-0 w-0 h-0.5 transition-all duration-300 group-hover:w-full ${link.isButton ? 'bg-white' : 'bg-[#703BF7]'
                    }`}
                ></span>
              </li>
            ))}
          ...
          <div
            className={`lg:hidden fixed top-8 left-0 w-full h-screen bg-[#141414] bg-opacity-95 transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'
            ...
            <div className="flex flex-col items-center justify-center h-full space-y-6 text-xl font-semibold">
              {[...navLinks, ...authLinks].map((link) => (
                <div
                  ...
                  className={`flex items-center gap-2 px-4 py-2 transition-all duration-300 ${link.isButton
