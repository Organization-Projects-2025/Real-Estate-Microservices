...
function loadImages() {
  ...
  return files.map((file) => {
    return {
      file,
      publicPath: `/images/${encodeURIComponent(file)}`,
    };
  });
}
...
const DeveloperProperty = mongoose.model(
  'DeveloperProperty',
  developerPropertySchema,
);
...
function propertySeed(image, userId, index, override = {}) {
  ...
  return {
    ...
    description: `A polished Egypt-focused ${sale ? 'sale' : 'rent'} listing in ${city}, close to daily services, main roads, and modern community facilities.`,
...
function devPropertySeed(
  ...
  return {
    ...
    projectId,
    title,
    description: `Primary unit in ${location}, designed for the Egyptian market with efficient layouts, secure access, and strong investment appeal.`,
...
async function seed() {
  ...
  const users = await User.insertMany(
    baseUsers.map(([firstName, lastName, email, role, phoneNumber], index) => ({
      ...
      authType: 'local',
      profilePicture: images[index % images.length].publicPath,
      role,
      phoneNumber,
  ...
  const agents = await Agent.insertMany(
    [
      [
        ...
        9,
        33,
        'New Cairo and New Capital specialist.',
      ],
      [
        'Omar',
        'Samir',
        ...
        7,
        35,
        'Sheikh Zayed and 6th of October advisor.',
      ],
      [
        'Youssef',
        'Adel',
        'agent3@realestate.com',
        91,
        11,
        39,
        'Commercial property and office leasing expert.',
      ],
      [
        'Laila',
        'Nabil',
        'agent4@realestate.com',
        42,
        6,
        31,
        'Maadi, Zamalek, and Nile-view homes consultant.',
      ],
      [
        'Hana',
        'Tarek',
        'agent5@realestate.com',
        67,
        8,
        34,
        'Alexandria and North Coast residential specialist.',
      ],
    ].map(
      ([
        firstName,
  ...
  const filters = await Filter.insertMany(
    ...
    ].map(([name, category, description, order]) => ({
      name,
      category,
      description,
      order,
  ...
  const developers = await Developer.insertMany([
    {
      ...
      description:
        'Egyptian developer focused on mixed-use communities in East Cairo.',
      contact: {
        email: 'hello@nilecrown.example',
        phone: '+20224000001',
        website: 'https://nilecrown.example',
      },
    },
    {
      name: 'Delta Heights Real Estate',
      description:
        'Residential and coastal communities across Giza, Alexandria, and the North Coast.',
      contact: {
        email: 'sales@deltaheights.example',
        phone: '+20224000002',
        website: 'https://deltaheights.example',
      },
    },
  ]);

  const developerUsers = [
    byEmail.get('developer1@realestate.com'),
    byEmail.get('developer2@realestate.com'),
  ];
  const projects = await Project.insertMany([
    {
      ...
      description:
        'A gated residential phase with parks, retail walkways, and smart home-ready apartments.',
      location: 'New Cairo, Cairo Governorate',
      images: [images[10].publicPath],
      status: 'active',
    ...
    },
    {
      ...
      description:
        'Administrative offices close to government district routes in the New Administrative Capital.',
      location: 'New Administrative Capital, Cairo Governorate',
      images: [images[11].publicPath],
      status: 'active',
    ...
    },
    {
      ...
      description:
        'Boutique apartments with green courtyards and fast access to Corniche routes.',
      location: 'Maadi, Cairo Governorate',
      images: [images[12].publicPath],
      status: 'active',
    ...
    },
    {
      ...
      description:
        'Family villas and duplexes near schools, clubs, and mall corridors in West Cairo.',
      location: 'Sheikh Zayed, Giza Governorate',
      images: [images[13].publicPath],
      status: 'active',
    ...
    },
    {
      ...
      description:
        'Mediterranean-inspired homes with serviced facilities near Alexandria Corniche.',
      location: 'Alexandria, Alexandria Governorate',
      images: [images[14].publicPath],
      status: 'active',
  ...
  const developerProperties = await DeveloperProperty.insertMany([
    devPropertySeed(
      images[15],
      projects[0].developerId,
      projects[0]._id,
      0,
      'Cairo Gate Three-Bedroom Apartment',
      projects[0].location,
    ),
    devPropertySeed(
      images[16],
      projects[1].developerId,
      projects[1]._id,
      1,
      'Capital Walk Finished Office',
      projects[1].location,
    ),
    devPropertySeed(
      images[17],
      projects[2].developerId,
      projects[2]._id,
      2,
      'Maadi Riverside Garden Duplex',
      projects[2].location,
    ),
    devPropertySeed(
      images[18],
      projects[3].developerId,
      projects[3]._id,
      3,
      'Zayed Palm Standalone Villa',
      projects[3].location,
    ),
    devPropertySeed(
      images[19],
      projects[4].developerId,
      projects[4]._id,
      4,
      'Alexandria Bay Sea-View Apartment',
      projects[4].location,
    ),
  ]);
  ...
  await User.updateOne(
    ...
    {
      savedProperties: standardProperties
        .slice(0, 3)
