...
describe('Developer Properties (converted)', function () {
  ...
  it('Developer projects page loads and lists projects', async function () {
    await driver.get(`${CLIENT_URL}/developer-properties`);
    await waitForPage(driver);
    const body = await driver.findElement(By.css('body')).getText();
    assert.match(body, /New Projects|Developer Properties|Projects/i);

    const links = await driver.findElements(By.css('a[href^="/developer/"]'));
    if (links.length) {
      await links[0].click();
      await waitForPage(driver);
      const details = await driver.findElement(By.css('body')).getText();
      assert.match(details, /Project|Location|Developer|Units/i);
    }
  });
});
